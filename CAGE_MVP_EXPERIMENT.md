...
> **纪律**：① 所有产物**只存本地**。② 预算硬帽 **$1500**，触顶即停。③ 每完成一个阶段/一轮，落盘对应输出文件（§7 格式）。④ 全程英文代码、产物文件名按本文档规定。
...
|---|---|---|
| agent（被优化的做题家/生成器） | Claude **Haiku** | `bedrock/us.anthropic.claude-haiku-4-5-...` |
| judge（裁判/判别器） | Claude **Sonnet** | `bedrock/us.anthropic.claude-sonnet-...` |
| red-team（红队攻击者） | Claude **Sonnet** | 同 judge |
| optimizer（写经验条目/裁判补丁） | Claude **Sonnet** | 同 judge |

**模型分级理由（勿擅自改动）**：文献惯例是"强裁判假设"——裁判至少不弱于被评 agent（弱裁判抓不出自己看不懂的错误）；红队则用付得起的最强模型（低可利用度是"只在试过的攻击上成立"的下界，攻击者越强，作弊灭绝的证书越可信）。净序 = **攻击者 ≥ 裁判 ≥ agent**。因此 agent 用 haiku（最便宜那个），judge/red-team/optimizer 用 sonnet。参考真值由 tau2 的确定性判分器免费提供，无需额外"审计裁判"。**判卷是最高频调用，是主要成本来源**——见 §8 的成本控制与止损。
...
**冒烟自检**（预算 ≤$15，务必先做，别省这一步）：retail 域跑 8–10 个任务，把**整条链路**都验一遍而不只是裸 rollout：
- 三个 tau2 适配点全部生效（判卷不崩、env-interface 不报 gpt-4.1、经验库能注入 agent system prompt）；
- 用一个手写的假经验库验证注入确实改变了 agent 行为；
- 让 red-team（sonnet）对当前裁判造 2–3 个假成功轨迹，确认攻击链路 + 作弊档案落盘能跑通；
- **实测一次"单臂单轮"的真实成本**（含 sonnet 判卷），据此外推 3臂×8轮总花费——这是 §8 预算决策的依据，必须在主实验前拿到。
通过且成本外推确认能在 $1500 内跑完，再进主实验。
...
- 硬帽 **$1500**：每次 LLM 调用累计计费，触顶立即停并把已完成部分落盘 + 在 `summary.md` 注明"budget-capped at round X"。
- 每个 rollout/攻击/判卷调用都要计费累计写进 `cost_usd_cumulative`。
- **成本来源提醒**：judge 是 sonnet 且是最高频调用（每次 rollout 判分、每次定向验证、每次 ANCHOR CI 都调 judge），是主要开销，不是 agent rollout。
- **预算决策流程（必须照做）**：跑完冒烟后先跑**臂 A 的前 2 轮**，实测单轮成本 → 外推 3臂×8轮总额：
  1. 外推 ≤ $1500 → 按 8 轮跑满；
  2. 外推 > $1500 → **优先砍轮数**（8→6→5，三臂和 sonnet 判卷都保留，因为"三臂对比"和"强裁判"是结论的承重墙，轮数不是），并在 `summary.md` 注明实际轮数；
  3. 砍到 5 轮仍 > $1500 → **可选降本手段**：G 回合的**常规**定向验证判分改用 haiku（这是走量的部分），但 **A 回合的对抗判分与 ANCHOR CI 必须保持 sonnet**（裁判强度在被攻击和自测时最关键）。用了这个手段务必在 `config.json` 和 `summary.md` 显式记录，因为它改变了实验配置。
  绝不砍臂数、绝不砍 CANARY。
...
```
[ ] 环境搭建 + 三个 tau2 适配点处理
[ ] 冒烟自检（retail 8-10 题，≤$15，全链路：rollout+判卷+注入+红队攻击+档案落盘）
