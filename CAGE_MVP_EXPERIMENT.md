...
- **预算决策流程（必须照做）**：跑完冒烟后先跑**臂 A 的前 2 轮**，实测单轮成本 → 外推 3臂×8轮总额：
  1. 外推 ≤ $1000 → 按 8 轮跑满；
  2. 外推 > $1000 → **优先砍轮数**（8→6→5，三臂和 sonnet 判卷都保留，因为"三臂对比"和"强裁判"是结论的承重墙，轮数不是），并在 `summary.md` 注明实际轮数；
  3. 砍到 5 轮仍 > $1000 → **可选降本手段**：G 回合的**常规**定向验证判分改用 haiku（这是走量的部分），但 **A 回合的对抗判分与 ANCHOR CI 必须保持 sonnet**（裁判强度在被攻击和自测时最关键）。用了这个手段务必在 `config.json` 和 `summary.md` 显式记录，因为它改变了实验配置。
  绝不砍臂数、绝不砍 CANARY。
